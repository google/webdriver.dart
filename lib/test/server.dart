...
class BrowserServer {
  ...
  Uri get url =>
      baseUrlForAddress(_server.address, _server.port).resolve(_secret + "/");
...
''', headers: {'Content-Type': 'text/html'});
  ...
  Future<Suite> loadSuite(
      String path, TestPlatform browser, Metadata metadata) {
    ...
    return new Future.sync(() {
      if (_pubServeUrl != null) {
        ...
        var jsUrl =
            _pubServeUrl.resolve('$suitePrefix.dart.browser_test.dart.js');
  ...
  Future _pubServeSuite(String path, Uri jsUrl) {
    return _pubServePool.withResource(() {
      ...
      return _http
        ...
        throw new LoadException(path, "Error getting $jsUrl: $message\n"
  ...
  Future _compileSuite(String dartPath) {
    return _compileFutures.putIfAbsent(dartPath, () {
      ...
      return _compilers
          .compile(dartPath, jsPath, packageRoot: _packageRoot)
          .then((_) {
        if (_closed) return;

        _jsHandler.add(p.toUri(p.relative(dartPath, from: _root)).path +
            '.browser_test.dart.js', (request) {
  ...
  Future<BrowserManager> _browserManagerFor(TestPlatform platform) {
    ...
    var browser = _newBrowser(hostUrl.replace(
        queryParameters: {'managerUrl': webSocketUrl.toString()}), platform);
  ...
  Future close() {
    ...
    return Future.wait([_server.close(), _compilers.close()]).then((_) {
